...
class CILogViewer 
{
	...
	const LOG_LINE_START_PATTERN = "/((ERROR)|(INFO)|(DEBUG)|(ALL)|(APP)|(USER))[\s\-\d:\.\/]+(-->)/";
	const LOG_DATE_PATTERN = ["/^((ERROR)|(INFO)|(DEBUG)|(ALL)|(APP)|(USER))\s\-\s/", "/\s(-->)/"];
	const LOG_LEVEL_PATTERN = "/^((ERROR)|(INFO)|(DEBUG)|(ALL)|(APP)|(USER))/";
	const LOG_FILE_PATTERN_CONFIG_KEY = "default_log_file_pattern";
	const LOG_FOLDER_PATH_CONFIG_KEY = "default_log_folder_path";
	const CI_LOG_VIEW_FILE_PATH = "Logs/logs_view";
	...
	private static $levelsIcon = [
		...
		'APP' => 'glyphicon glyphicon-minus',
		'USER' => 'glyphicon glyphicon-minus',
	];
	...
	{
		use_config('Logs/Config');

		$this->useLog();
	}

	public function useLog($default = 'default')
	{
	...
	private function systemLogs()
	{
		...
		$this->logFolderPath = ! is_null($this->CI->config->item('default_log_folder_path')) ? rtrim($this->CI->config->item('default_log_folder_path'), "/") : rtrim(APPPATH, "/") . "/logs";
		$this->logFilePattern = ! is_null($this->CI->config->item('default_log_file_pattern')) ? $this->CI->config->item('default_log_file_pattern') : "log-*.php";

		//concatenate to form Full Log Path
		$this->fullLogFilePath = $this->logFolderPath . "/" . $this->logFilePattern;
	}
	
	private function appLogs()
	{
		...
		$this->logFolderPath = ! is_null($this->CI->config->item('app_log_folder_path')) ? rtrim($this->CI->config->item('app_log_folder_path'), "/") : rtrim(APPPATH, "/") . "/logs";
		$this->logFilePattern = ! is_null($this->CI->config->item('app_log_file_pattern')) ? $this->CI->config->item('app_log_file_pattern') : "log-*.php";
	...
	private function userLogs()
	{
		...
		$this->logFolderPath = ! is_null($this->CI->config->item('user_log_folder_path')) ? rtrim($this->CI->config->item('user_log_folder_path'), "/") : rtrim(APPPATH, "/") . "/logs";
		$this->logFilePattern = ! is_null($this->CI->config->item('user_log_file_pattern')) ? $this->CI->config->item('user_log_file_pattern') : "log-*.php";
